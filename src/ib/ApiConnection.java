...
import ib.Types.AlgoStrategy;
import ib.Types.HedgeType;
...
public class ApiConnection {
	...
	// 60 = can receive deltaNeutralOpenClose, deltaNeutralShortSale, deltaNeutralShortSaleSlot and deltaNeutralDesignatedLocation in
	// openOrder
	...
	private IMessageHandler m_messageHandler; // msg handler
	...
	private MessageReaderThread m_reader; // thread which reads msgs from socket

	/**
	 * 
	 * @return
	 */
	public MessageReaderThread reader() {
		return m_reader;
	}
	 ...
	 * @param msg_handler
	 */
	public ApiConnection(IMessageHandler msg_handler) {
		m_messageHandler = msg_handler;
		m_clientId = -1;
		m_extraAuth = false;
		m_connected = false;
		m_serverVersion = 0;
	...
	protected void connectionError() {
		m_messageHandler.error(EClientErrors.NO_VALID_ID, EClientErrors.CONNECT_FAIL.code(), EClientErrors.CONNECT_FAIL.msg());
		m_reader = null;
	}
	...
	protected String checkConnected(String host) {
		if (m_connected) {
			m_messageHandler.error(EClientErrors.NO_VALID_ID, EClientErrors.ALREADY_CONNECTED.code(), EClientErrors.ALREADY_CONNECTED.msg());
			return null;
		}
		if (isNull(host)) {
			host = "127.0.0.1";
	 ...
	 * @return
	 */
	public MessageReaderThread createReader(ApiConnection socket, DataInputStream dis) {
		return new MessageReaderThread(socket, dis);
	}
	...
	public synchronized void eConnect(Socket socket) throws IOException {
		...
		if (m_serverVersion < SERVER_VERSION) {
			eDisconnect();
			m_messageHandler.error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS.code(), EClientErrors.UPDATE_TWS.msg());
			return;
		}
	...
	public synchronized void eDisconnect() {
		...
		MessageReaderThread reader = m_reader;
		m_reader = null;
	...
	public synchronized void placeOrder(int id, Contract contract, Order order) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
			if (order.m_deltaNeutralConId > 0 || !IsEmpty(order.m_deltaNeutralSettlingFirm) || !IsEmpty(order.m_deltaNeutralClearingAccount) || !IsEmpty(order.m_deltaNeutralClearingIntent)) {
		...
		if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_OPEN_CLOSE) {
			if (!IsEmpty(order.m_deltaNeutralOpenClose) || order.m_deltaNeutralShortSale || order.m_deltaNeutralShortSaleSlot > 0 || !IsEmpty(order.m_deltaNeutralDesignatedLocation)) {
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3) {
			if (order.m_scalePriceIncrement > 0 && order.m_scalePriceIncrement != Double.MAX_VALUE) {
				if (order.m_scalePriceAdjustValue != Double.MAX_VALUE || order.m_scalePriceAdjustInterval != Integer.MAX_VALUE || order.m_scaleProfitOffset != Double.MAX_VALUE || order.m_scaleAutoReset || order.m_scaleInitPosition != Integer.MAX_VALUE
						|| order.m_scaleInitFillQty != Integer.MAX_VALUE || order.m_scaleRandomPercent) {
					error(id, EClientErrors.UPDATE_TWS, "  It does not support Scale order parameters: PriceAdjustValue, PriceAdjustInterval, " + "ProfitOffset, AutoReset, InitPosition, InitFillQty and RandomPercent");
					return;
				}
			}
		}
		...
		if (m_serverVersion < MIN_SERVER_VER_SCALE_TABLE) {
			if (!IsEmpty(order.m_scaleTable) || !IsEmpty(order.m_activeStartTime) || !IsEmpty(order.m_activeStopTime)) {
				error(id, EClientErrors.UPDATE_TWS, "  It does not support scaleTable, activeStartTime and activeStopTime parameters.");
				return;
			}
		}
	...
	public synchronized void verifyMessage(String apiData) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support verification message sending.");
			return;
		}
	...
	public synchronized void queryDisplayGroups(int reqId) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support queryDisplayGroups request.");
			return;
		}
	...
	public synchronized void subscribeToGroupEvents(int reqId, int groupId) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support subscribeToGroupEvents request.");
			return;
		}
	...
	public synchronized void updateDisplayGroup(int reqId, String contractInfo) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support updateDisplayGroup request.");
			return;
		}
	...
	public synchronized void unsubscribeFromGroupEvents(int reqId) {
		...
		if (m_serverVersion < MIN_SERVER_VER_LINKING) {
			error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support unsubscribeFromGroupEvents request.");
			return;
		}
	...
	/** @deprecated, never called (except for version < 2 haha). */
	protected synchronized void error(String err) {
		m_messageHandler.error(err);
	}

	protected synchronized void error(int id, int errorCode, String errorMsg) {
		m_messageHandler.error(id, errorCode, errorMsg);
	}

	protected void close() {
		eDisconnect();
		getMessageHandler().connectionClosed();
	}
