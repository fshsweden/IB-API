...
public class MessageReaderThread extends Thread {
	...
	/*
	 * Owner is a ApiConnection
	 * Owner has a pointer to a message handler (usually ApiController)
	 */
	private ApiConnection	m_owner;
	private DataInputStream	m_dis;

	protected ApiConnection owner() {
		return m_owner;
	}

	private IMessageHandler getMessageHandler() {
		return owner().getMessageHandler();
	}

	public MessageReaderThread(ApiConnection conn, DataInputStream dis) {
		this("MessageReader", conn, dis);
	}

	protected MessageReaderThread(String name, ApiConnection conn, DataInputStream dis) {
		setName(name);
		m_owner = conn;
		m_dis = dis;
	}
	...
	protected boolean processMsg(int msgId) throws IOException {
		...
		switch (msgId) {
			...
			case TICK_PRICE: {
				...
				getMessageHandler().tickPrice(tickerId, tickType, price, canAutoExecute);
				...
				if (version >= 2) {
					...
					if (sizeTickType != -1) {
						getMessageHandler().tickSize(tickerId, sizeTickType, size);
					}
				}
				break;
			}
			...
			case TICK_SIZE: {
				...
				getMessageHandler().tickSize(tickerId, tickType, size);
				break;
			}
			...
			case POSITION: {
				...
				getMessageHandler().position(account, contract, pos, avgCost);
				break;
			}

			case POSITION_END: {
				int version = readInt();
				getMessageHandler().positionEnd();
				break;
			}
			...
			case ACCOUNT_SUMMARY: {
				...
				getMessageHandler().accountSummary(reqId, account, tag, value, currency);
				break;
			}
			...
			case ACCOUNT_SUMMARY_END: {
				int version = readInt();
				int reqId = readInt();
				getMessageHandler().accountSummaryEnd(reqId);
				break;
			}
			...
			case TICK_OPTION_COMPUTATION: {
				...
				getMessageHandler().tickOptionComputation(tickerId, tickType, impliedVol, delta, optPrice, pvDividend, gamma, vega, theta, undPrice);
				break;
			}
			...
			case TICK_GENERIC: {
				...
				getMessageHandler().tickGeneric(tickerId, tickType, value);
				break;
			}
			...
			case TICK_STRING: {
				...
				getMessageHandler().tickString(tickerId, tickType, value);
				break;
			}
			...
			case TICK_EFP: {
				...
				getMessageHandler().tickEFP(tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays, futureExpiry, dividendImpact, dividendsToExpiry);
				break;
			}
			...
			case ORDER_STATUS: {
				...
				getMessageHandler().orderStatus(id, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld);
				break;
			}
			...
			case ACCT_VALUE: {
				...
				getMessageHandler().updateAccountValue(key, val, cur, accountName);
				break;
			}
			...
			case PORTFOLIO_VALUE: {
				...
				if (version == 6 && m_owner.serverVersion() == 39) {
					contract.m_primaryExch = readStr();
				}

				getMessageHandler().updatePortfolio(contract, position, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL, accountName);
			...
			case ACCT_UPDATE_TIME: {
				...
				getMessageHandler().updateAccountTime(timeStamp);
				break;
			}
			...
			case ERR_MSG: {
				...
				if (version < 2) {
					...
					m_owner.error(id, errorCode, errorMsg);
				}
				break;
			}
			...
			case OPEN_ORDER: {
				...
				getMessageHandler().openOrder(order.m_orderId, contract, order, orderState);
				break;
			}
			...
			case NEXT_VALID_ID: {
				int version = readInt();
				int orderId = readInt();
				getMessageHandler().nextValidId(orderId);
				break;
			}
			...
			case SCANNER_DATA: {
				...
				for (int ctr = 0; ctr < numberOfElements; ctr++) {
					...
					getMessageHandler().scannerData(tickerId, rank, contract, distance, benchmark, projection, legsStr);
				}
				getMessageHandler().scannerDataEnd(tickerId);
				break;
			}
			...
			case CONTRACT_DATA: {
				...
				getMessageHandler().contractDetails(reqId, contract);
				break;
			}
			case BOND_CONTRACT_DATA: {
				...
				getMessageHandler().bondContractDetails(reqId, contract);
				break;
			}
			case EXECUTION_DATA: {
				int version = readInt();
				...
				getMessageHandler().execDetails(reqId, contract, exec);
				break;
			}
			case MARKET_DEPTH: {
				int version = readInt();
				...
				getMessageHandler().updateMktDepth(id, position, operation, side, price, size);
				break;
			}
			case MARKET_DEPTH_L2: {
				...
				getMessageHandler().updateMktDepthL2(id, position, marketMaker, operation, side, price, size);
				break;
			}
			case NEWS_BULLETINS: {
				int version = readInt();
				...
				getMessageHandler().updateNewsBulletin(newsMsgId, newsMsgType, newsMessage, originatingExch);
				break;
			}
			case MANAGED_ACCTS: {
				int version = readInt();
				...
				getMessageHandler().managedAccounts(accountsList);
				break;
			}
			case RECEIVE_FA: {
				int version = readInt();
				...
				getMessageHandler().receiveFA(faDataType, xml);
				break;
			}
			case HISTORICAL_DATA: {
				...
				for (int ctr = 0; ctr < itemCount; ctr++) {
					...
					getMessageHandler().historicalData(reqId, date, open, high, low, close, volume, barCount, WAP, Boolean.valueOf(hasGaps).booleanValue());
				}
				// send end of dataset marker
				getMessageHandler().historicalData(reqId, completedIndicator, -1, -1, -1, -1, -1, -1, -1, false);
				break;
			}
			case SCANNER_PARAMETERS: {
				int version = readInt();
				String xml = readStr();
				getMessageHandler().scannerParameters(xml);
				break;
			}
			case CURRENT_TIME: {
				/* int version = */ readInt();
				long time = readLong();
				getMessageHandler().currentTime(time);
				break;
			}
			case REAL_TIME_BARS: {
				/* int version = */ readInt();
				...
				double wap = readDouble();
				int count = readInt();
				getMessageHandler().realtimeBar(reqId, time, open, high, low, close, volume, wap, count);
				break;
			}
			case FUNDAMENTAL_DATA: {
				...
				int reqId = readInt();
				String data = readStr();
				getMessageHandler().fundamentalData(reqId, data);
				break;
			}
			case CONTRACT_DATA_END: {
				/* int version = */ readInt();
				int reqId = readInt();
				getMessageHandler().contractDetailsEnd(reqId);
				break;
			}
			case OPEN_ORDER_END: {
				/* int version = */ readInt();
				getMessageHandler().openOrderEnd();
				break;
			}
			case ACCT_DOWNLOAD_END: {
				/* int version = */ readInt();
				String accountName = readStr();
				getMessageHandler().accountDownloadEnd(accountName);
				break;
			}
			case EXECUTION_DATA_END: {
				/* int version = */ readInt();
				int reqId = readInt();
				getMessageHandler().execDetailsEnd(reqId);
				break;
			}
			case DELTA_NEUTRAL_VALIDATION: {
				...
				getMessageHandler().deltaNeutralValidation(reqId, underComp);
				break;
			}
			case TICK_SNAPSHOT_END: {
				...
				getMessageHandler().tickSnapshotEnd(reqId);
				break;
			}
			case MARKET_DATA_TYPE: {
				...
				getMessageHandler().marketDataType(reqId, marketDataType);
				break;
			}
			case COMMISSION_REPORT: {
				...
				getMessageHandler().commissionReport(commissionReport);
				break;
			}
			case VERIFY_MESSAGE_API: {
				...
				getMessageHandler().verifyMessageAPI(apiData);
				break;
			}
			case VERIFY_COMPLETED: {
				...
				getMessageHandler().verifyCompleted(isSuccessful, errorText);
				break;
			}
			case DISPLAY_GROUP_LIST: {
				...
				getMessageHandler().displayGroupList(reqId, groups);
				break;
			}
			case DISPLAY_GROUP_UPDATED: {
				...
				getMessageHandler().displayGroupUpdated(reqId, contractInfo);
				break;
			}

			default: {
				m_owner.error(EClientErrors.NO_VALID_ID, EClientErrors.UNKNOWN_ID.code(), EClientErrors.UNKNOWN_ID.msg());
				return false;
			}
		}
		return true;
