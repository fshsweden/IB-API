...
import ib.Types.DeepSide;
import ib.Types.DeepType;
import ib.Types.DurationUnit;
import ib.Types.ExerciseType;
import ib.Types.FADataType;
import ib.Types.FundamentalType;
import ib.Types.MktDataType;
import ib.Types.NewsType;
import ib.Types.WhatToShow;

public class ApiController implements IMessageHandler {
	...
	/*
	 * Changed m_orderHandlers to contain a LIST of handlers, since it si possible that we have multiple ones "alive" against the same order
	 * id! 
	 * NOTE: Do NOT use removeOrderHandler in a callback, since the caller is probably iterating the list of handlers!
	 * A possible (inefficient) solution is to clone the list of handlers before iterating....
	 */
	private final HashMap<Integer, List<IOrderHandler>>			m_orderHandlers			= new HashMap<Integer, List<IOrderHandler>>();
	...
	public interface IConnectionHandler {
		void connected(final int nextValidOrderId);
		void disconnected();
	...
	public void nextValidId(int orderId) {
		...
		if (m_connectionHandler != null) {
			m_connectionHandler.connected(orderId);
		}
	}
	...
	public void error(int id, int errorCode, String errorMsg) {
		...
		// Iterate a copy since callback might call removeHandler()!
		List<IOrderHandler> handlers = m_orderHandlers.get(id);
		if (handlers != null) {
			List<IOrderHandler> handlers_copy = new ArrayList<IOrderHandler>();
			handlers_copy.addAll(handlers);

			for (IOrderHandler h : handlers_copy) {
				h.handle(errorCode, errorMsg);
			}
	...
	public int placeOrModifyOrder(NewContract contract, final NewOrder order, final IOrderHandler handler) {
		...
		// How to diff between ADD and MODIFY handlers?
		if (handler != null) {
			List<IOrderHandler> h = m_orderHandlers.get(order.orderId());
			if (h == null) {
				h = new ArrayList<IOrderHandler>();
				m_orderHandlers.put(order.orderId(), h);
			}
			h.add(handler);
		}
	...
	public void removeOrderHandler(int orderId, IOrderHandler handler) {
		List<IOrderHandler> handlers = m_orderHandlers.get(orderId);
		if (handlers != null) {
	...
	public void openOrder(int orderId, Contract contract, Order orderIn, OrderState orderState) {
		...
		if (handlers != null) {
			...
			for (IOrderHandler handler : handlers_copy) {
